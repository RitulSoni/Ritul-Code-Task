...
class QuestionMuniPair:
    ...
    def parse_answer(self, answer, location):
        try:
            ...
            answer_str, confidence_str = self.extract_answer_and_confidence(answer)
            parsed_answer = self.process_answer(answer_str)
            confidence_level = self.process_confidence(confidence_str)

            #Dont know variable is whether parsed answer is None
            dont_know = parsed_answer is None

            return {'Answer': parsed_answer, 'Dont_Know': dont_know,'Confidence': confidence_level}
    ...
    def extract_answer_and_confidence(self, text):
        # Split text to extract the answer and confidence parts
        answer_part, confidence_part = text.split('CONFIDENCE:', 1)
        answer_str = answer_part.split('ANSWER:', 1)[1].strip()
        confidence_str = confidence_part.strip()
        return answer_str, confidence_str
